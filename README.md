...
> 📦 **当前版本：v1.3.5**（2026-09-04）—— 账号先行主路固化（先爬自己的号） + demo 红线（主路唯一） + 首次运行人闸引导 + 运营说人话 SOP
...
> ⚠️ **如果你是 AI 助手或新手，第一次跑本项目：请先读 [首次运行·人闸引导](docs/methodology/首次运行_人闸引导_模板.md)** —— 一个阶段一个阶段来，每阶段停下等人确认，不要一口气全自动跑完（2026-09-04 起强制）。
> 🗺️ **主路唯一（账号先行）**：本仓库的正确主流程是 **先爬「自己的账号」→ 风格框架 → 打分卡选方向 → 爬爆款 → 选题 → 成稿**，不是先爬竞品。
>    完整路径图与阶段说明见 [docs/methodology/README.md](docs/methodology/README.md)「0. 主路唯一：账号先行」。
