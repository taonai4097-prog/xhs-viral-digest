...
async function doSaveDraft(payload) {
  ...
  try {
    ...
    // 标题
    await page.fill("#title", payload.title || "", { timeout: 10000 })
      .catch((e) => log("warn", "填标题失败", { err: String(e.message) }));
    // 正文（contenteditable 编辑器）
    await page.click(".ql-editor", { timeout: 8000 }).catch(() => {});
    await page.keyboard.type(payload.content || "", { delay: 5 }).catch(() => {});
    // 话题标签（XHS 在编辑器内打 # 自动转话题）
    ...
    if (imgs.length) {
      ...
      if (input) {
        await input.setInputFiles(imgs).catch((e) => log("warn", "传图失败", { err: String(e.message) }));
        await page.waitForTimeout(1500);
      }
    }
    // 存草稿
    await page.click("text=存草稿", { timeout: 10000 })
      .catch((e) => log("warn", "点存草稿失败", { err: String(e.message) }));
    await page.waitForTimeout(3000);
    return {
      success: true, code: "DRAFT_SAVED",
      data: { note_id: "draft-" + Date.now(), images: imgs.length },
      message: "草稿已存入草稿箱，请在 App 里人工终审发布",
    };
  } finally {
